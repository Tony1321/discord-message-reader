...
####Как работает
Бот подключается к серверам, где он есть

Сканирует текстовые каналы, кроме игнорируемых

Если указан SCAN_CHANNELS.txt, сканирует только эти каналы

Для каждого сообщения сохраняется: [канал] дата-время — текст сообщения
